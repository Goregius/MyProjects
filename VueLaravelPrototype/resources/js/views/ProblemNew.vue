<template>
    <b-card header-tag="header">
        <h2 slot="header" class="mb-0">New Problem</h2>
        <b-form @submit.prevent>
            ...
            <b-button type="submit" :disabled="!showResult" :variant="showResult ? 'primary' : 'disabled'">Report</b-button>
        </b-form>
    </b-card>
</template>
    ...
    methods: {
        ...
        updateSpecialists() {
            ...
            while (this.form.specialists.length === 0) {
                this.form.specialists = specialists.filter(spec =>
                    types.slice(0, types.length - 1).every(type => spec.problemTypes.includes(type))
                );
            }

            this.form.specialists = this.form.specialists.sort((a, b) => a.jobs < b.jobs);
